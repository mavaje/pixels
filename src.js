...
var CONSTANTS, assert, assertionError, stringToByteArray$1, byteArrayToString, base64, DecodeBase64StringError, base64Encode, base64urlEncodeWithoutPadding, base64Decode, getDefaultsFromGlobal, getDefaultsFromEnvVariable, getDefaultsFromCookie, getDefaults, getDefaultEmulatorHost, getDefaultEmulatorHostnameAndPort, getDefaultAppConfig, Deferred, emulatorStatus, previouslyDismissed, ERROR_NAME, FirebaseError, ErrorFactory, PATTERN, decode, isValidFormat, isAdmin, Sha1, stringToByteArray, stringLength, MAX_VALUE_MILLIS;
...
var init_app = __esm({
  "db/app.ts"() {
    init_index_esm6();
    app = initializeApp({
      apiKey: "AIzaSyDqaoNp2fmQ1DN9UXO59OrbYk9-OSJMGZY",
      authDomain: "mavaje-pixels.firebaseapp.com",
      databaseURL: "https://mavaje-pixels-default-rtdb.firebaseio.com",
      projectId: "mavaje-pixels",
      storageBucket: "mavaje-pixels.firebasestorage.app",
      messagingSenderId: "355949515699",
      appId: "1:355949515699:web:2c166540a64faad9eab1c6"
    });
  }
});
  ...
  "palette.ts"() {
    ...
    Palette = class {
      ...
      static initialise() {
        ...
        if (hexes) {
          hexes.split(",").map((hex) => Colour.clean_hex(hex, true)).slice(0, 3).forEach((hex, i) => {
            this.set_pip(i, hex);
          });
        }
...
// tools/pick-tool.ts
var PickTool, pick_tool;
var init_pick_tool = __esm({
  "tools/pick-tool.ts"() {
    init_tool();
    init_block();
    init_palette();
...
function on_touch(event) {
  ...
  const preview_visible = event.target === PixelGrid.canvas && !multi_touched && tool.preview_visible();
